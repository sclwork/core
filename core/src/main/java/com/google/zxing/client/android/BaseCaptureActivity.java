...
public abstract class BaseCaptureActivity
        extends BaseActivity implements TextureView.SurfaceTextureListener {
...
//                source = IntentSource.NATIVE_APP_INTENT;
        ...
        if (Permission.hasCameraPermission(new OnCheckPermissionImpl(this))) {
            reopenCamera();
        }
    }
    ...
    protected void onPause(Bundle args) {
        ...
        beepManager.close();
        if (!hasSurface) {
            TextureView surfaceView = getTextureView();
            if (surfaceView == null) {
    ...
    protected void reopenCamera() {
        ...
        if (hasSurface) {
            ...
            initCamera(surfaceTexture, rect);
        } else {
...
//            Log.i(TAG, "decodeOrStoreSavedBitmap ~~~~~~~ not yet have ");
    ...
    private void initCamera(SurfaceTexture surfaceTexture, Rect rect) {
        ...
        try {
            cameraManager.openDriver(surfaceTexture, rect);
